          ...
          <div class="card-body card-dashboard">
            <div class="table-responsive">
              <table class="text-dark table table-striped dataex-html5-selectors">
                ...
                <tr>
                  <td>{{$order->id}}</td>
                  <td>{{$order->user->name}} @foreach($order->orderdetail as $detail) @php $total+=$detail->price-($detail->discount/100)*$detail->price @endphp @endforeach</td>
                  <td>{{$total}}</td>
                  
                  <td>
                  <a href="{{route('vendor.order.details',['id'=>$order->id])}}" class="text-white bg-success">View</a>
